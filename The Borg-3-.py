...
# Define the main function to integrate all components
...
def main(file_path):
    ...
    anomalies_train = detect_anomalies(pd.DataFrame(X_train))
    X_train = handle_anomalies(pd.DataFrame(X_train), anomalies_train)
    y_train = y_train[~anomalies_train]
    ...
    tflite_model_path = convert_to_tflite(best_model, (1, X_train.shape[1]))
