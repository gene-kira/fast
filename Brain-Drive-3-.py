...
selected_drives = []
drive_vars = {}
drive_menu_frame = None

def get_available_drives():
    partitions = psutil.disk_partitions()
    return [p.device for p in partitions if 'rw' in p.opts]

def select_multiple_drives():
    global selected_drives
    selected_drives = [d for d, var in drive_vars.items() if var.get()]
    if selected_drives:
        with open("neuro_mount.cfg", "w") as f:
            for d in selected_drives:
                f.write(f"Selected Brain Drive: {d}\n")
        status_label.config(text=f"🧠 Mounted Drives: {', '.join(selected_drives)}")
        messagebox.showinfo("Drives Activated", f"Parallel Brain Network Engaged:\n{', '.join(selected_drives)}")
    else:
        messagebox.showerror("Error", "Please select at least one drive.")

def rescan_drives():
    global drive_menu_frame
    drive_menu_frame.destroy()
    setup_drive_selector()

def setup_drive_selector():
    global drive_vars, drive_menu_frame
    drives = get_available_drives()
    drive_vars = {d: tk.BooleanVar() for d in drives}
    drive_menu_frame = tk.Frame(root, bg="#2e2e2e")
    drive_menu_frame.pack()

    for d in drives:
        tk.Checkbutton(drive_menu_frame, text=d, variable=drive_vars[d],
                       font=("Arial", 10), bg="#424242", fg="white",
                       selectcolor="#616161").pack(anchor="w", padx=30)

    tk.Button(drive_menu_frame, text="⚙️ Mount & Activate", command=select_multiple_drives,
...
def launch_magicbox():
    ...
    # 💾 Memory Chamber - Multi-Drive Selector
    ...
    canvas = tk.Canvas(root, width=500, height=200, bg="#37474f")
    canvas.pack()
