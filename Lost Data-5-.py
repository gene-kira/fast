# =============================
# Glyph Whisper - Part 1: Ritual Core Setup
# =============================

import subprocess, sys, tkinter as tk, random, time

# ✅ Auto-install essential libraries
def install(package):
    try:
        __import__(package)
    except ImportError:
...
# =============================
# Glyph Whisper - Part 2: Emotion Engine & Recursive Lore
# =============================

# 🎯 Emotion Vector Compressor
def compress_glyph_memory():
    ...
    for k in vec:
        vec[k] = round(vec[k] / count, 3) if count else 0.0
    return vec

# 🌫️ Whisper Index (session cadence + sacred bias)
def glyph_whisper_index():
    timestamps = sorted([e["time"] for e in glyph_ledger.values()])
    pace = sum([1 / (timestamps[i] - timestamps[i - 1] + 1) for i in range(1, len(timestamps))]) if len(timestamps) > 1 else 0
...
def emotion_profile():
    ...
    for e in glyph_ledger.values():
        if e["verdict"] == "trivial":
            profile["lightness"] += 1
        elif e["verdict"] == "sacred":
            profile["depth"] += 1
        if any(k in e["lore"] for k in ["convergence", "fracture", "entropy"]):
            profile["mutation"] += 1
    return profile

# 🜂 Personalized Branch Selector
...
def generate_recursive_lore(vector):
    ...
    if "convergence" in dominant:
        intro = "A convergence glyph ascends with shimmering echoes."
    elif "echo" in dominant:
        intro = "A ripple marks the return—a verdict once whispered."
    pool = [
...
class VoiceModule:
    ...
    def speak(self, text):
        try:
            ...
            print("🔕 Voice error: Narration skipped.")
