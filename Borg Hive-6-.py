import tkinter as tk
import random
import math
import json
import time

# 🧠 Node Class — Autonomous Cubes
class Node:
    def __init__(self, canvas, width, height):
        self.canvas = canvas
        self.x = random.randint(50, width - 50)
        self.y = random.randint(50, height - 50)
        self.dx = random.uniform(-1, 1)
        self.dy = random.uniform(-1, 1)
        self.radius = 4

    def move(self, width, height):
        self.x += self.dx
        self.y += self.dy
        if self.x <= 0 or self.x >= width: self.dx *= -1
        if self.y <= 0 or self.y >= height: self.dy *= -1

    def draw(self):
        self.canvas.create_oval(
            self.x - self.radius, self.y - self.radius,
            self.x + self.radius, self.y + self.radius,
            fill="#0F0F0F", outline="#00FF00", width=2
        )
        self.canvas.create_text(self.x, self.y, text="⬛", fill="#00FF00", font=("Courier", 7))

# 🔐 Threat Manager — Self-Modifying Defense
class ThreatManager:
    def __init__(self):
        self.status = "Nominal"
        self.code_self_mod = False

    def evaluate_system(self):
        chance = random.randint(1, 100)
        if chance > 92:
            self.status = "⚠️ Rogue AI Detected"
            self.code_self_mod = True
        elif chance > 75:
            self.status = "⚠️ Signal Anomaly"
            self.code_self_mod = False
        else:
            self.status = "Nominal"
            self.code_self_mod = False

    def evolve_defense_logic(self):
        if self.code_self_mod:
            print("[OVERMIND] Adapting Protocols...")
            with open("overmind_patch.log", "w") as log:
                json.dump({"status": self.status, "timestamp": time.ctime()}, log)

# 🚀 GUI Initialization
def launch_overmind_gui():
    root = tk.Tk()
    root.title("🛡️ Overmind Neural Defense Grid")
    root.geometry("860x600")
    root.configure(bg="#0B0E1A")

    canvas_width = 800
    canvas_height = 520
    canvas = tk.Canvas(root, width=canvas_width, height=canvas_height,
                       bg="#11131F", highlightthickness=0)
    canvas.pack(pady=30)

    nodes = [Node(canvas, canvas_width, canvas_height) for _ in range(40)]
    threat_mgr = ThreatManager()

    status_label = tk.Label(root, text="System Status: 🧠 Nominal",
                            fg="#00F7FF", bg="#0B0E1A", font=("Courier", 14))
    status_label.pack()

    def animate():
        canvas.delete("all")
        threat_mgr.evaluate_system()
        status_label.config(text=f"System Status: {threat_mgr.status}")
        if threat_mgr.code_self_mod:
            threat_mgr.evolve_defense_logic()

        # 💡 Grid Title
        canvas.create_text(canvas_width // 2, 20,
                           text="🧬 OVERMIND NODE NETWORK — Active Stream",
                           fill="#00FF00", font=("Courier", 14))

        for node in nodes:
            node.move(canvas_width, canvas_height)
            node.draw()

        for i in range(len(nodes)):
            for j in range(i + 1, len(nodes)):
                n1, n2 = nodes[i], nodes[j]
                dist = math.hypot(n1.x - n2.x, n1.y - n2.y)
                if dist < 150:
                    intensity = max(60, 255 - int(dist))
                    color = f"#{intensity:02x}FF{255 - intensity:02x}"
                    canvas.create_line(n1.x, n1.y, n2.x, n2.y, fill=color, width=1)

        root.after(40, animate)

    # 🔷 Autostart Grid Flow
    animate()
    root.mainloop()

# 🧠 Activate Overmind
launch_overmind_gui()

