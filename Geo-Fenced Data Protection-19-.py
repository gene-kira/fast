...
tk.Button(button_row, text="Epochs 📜", bg="navy", fg="white",
          command=show_epoch_chronicle).pack(side="left", padx=4)



tk.Button(button_row, text="Portrait 🎭", bg="red", fg="white",
          command=lambda: show_glyph_portrait("Seeker_453")).pack(side="left", padx=4)

tk.Button(button_row, text="Family Rings 🌀", bg="darkgreen", fg="white",
          command=show_family_rings).pack(side="left", padx=4)

tk.Button(button_row, text="Mood Log 📜", bg="darkblue", fg="white",
          command=show_mood_chronicle).pack(side="left", padx=4)
...
import hashlib, psutil, keyboard
...
def initialize_file_hashes():
    for path in [MEMORY_FILE, VERDICT_LOG]:
        file_hashes[path] = hash_file(path)

def check_file_integrity():
    for path in file_hashes:
        current = hash_file(path)
        if current != file_hashes[path]:
            glyph_memory.setdefault("audit", []).append({
                "timestamp": datetime.datetime.now().isoformat(),
                "event": "FILE_TAMPER_ALERT",
                "file": path,
                "expected": file_hashes[path],
                "current": current
            })
            print(f"🛡️ Tampering detected in {path}")
            file_hashes[path] = current  # Update to avoid spam

# 🌐 External System Quarantine
quarantine_set = set()
def quarantine_glyph(gid, ip_port):
    quarantine_set.add(ip_port)
    glyph_memory.setdefault("audit", []).append({
        "timestamp": datetime.datetime.now().isoformat(),
        "event": "QUARANTINE_GLYPH",
        "target": gid,
        "ip_port": ip_port
    })

def is_quarantined(ip_port):
    return ip_port in quarantine_set

# 🧿 Behavioral Entropy Score
glyph_entropy = {}
def compute_entropy(gid):
    events = glyph_memory.get("graph", {}).get(gid, [])
    if not events: return 0
    verdicts = [e["verdict"] for e in events if e.get("verdict")]
    flips = sum(1 for i in range(1, len(verdicts)) if verdicts[i] != verdicts[i-1])
    entropy = flips / len(verdicts)
    glyph_entropy[gid] = entropy
    return entropy

def show_entropy_scores():
    for gid in glyph_memory.get("graph", {}):
        ent = compute_entropy(gid)
        print(f"🧠 Glyph: {gid} — Entropy: {ent:.2f}")

# 📜 Guardian Ritual Table
def get_latest_verdict(gid):
    log = load_verdict_log()
    recent = [v for v in reversed(log) if v["system_id"] == gid]
    return recent[0]["verdict"] if recent else "Home"

def guardian_posture():
    verdict = get_latest_verdict(local_system_id)
    if verdict == "Self-Destruction":
        seal_purge_protocol()
...
def seal_purge_protocol():
    print("🧿 Protocol purge initiated")
    glyph_memory["threats"] = []
    glyph_memory["known_nodes"] = {}
    with open(MEMORY_FILE, "w") as f:
        json.dump(glyph_memory, f, indent=2)
...
def secure_open(path, mode="r"):
    verdict = get_latest_verdict(local_system_id)
    if verdict in ["Self-Destruction", "Tracking"]:
        print(f"🛡️ Access denied to {path} due to verdict: {verdict}")
        return None
    return open(path, mode)

# ⌨️ Key Monitoring for Emergency Protocol
def monitor_keystrokes():
    def trigger(event):
        if event.name in ["f12", "ctrl", "alt"]:
            print("🧿 Emergency key detected — initiating purge")
            seal_purge_protocol()
    keyboard.on_press(trigger)

# ⛓️ Watch Glyph Ports
def watch_glyph_ports():
    for conn in psutil.net_connections(kind='inet'):
        if conn.laddr.port in COMMON_PORTS and conn.status == "LISTEN":
            ...
            print(f"🛡️ Glyph port active: {conn.laddr.port} by PID {pid}")
...
def assign_personality(gid):
    ...
    if entropy > 0.6:
        return {"tone": "chaotic", "emotion": "intense", "style": "erratic", "color": "#FF3333"}
    elif entropy < 0.2:
...
def broadcast_verdict(gid, verdict):
    msg = f"VERDICT_BROADCAST::{gid}::{verdict}::{datetime.datetime.now().isoformat()}"
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    s.sendto(msg.encode(), ("255.255.255.255", 45203))
    s.close()

def listen_for_verdicts():
    def loop():
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.bind(("", 45203))
        while True:
            try:
                ...
                if parts[0] == "VERDICT_BROADCAST":
                    gid, verdict, timestamp = parts[1], parts[2], parts[3]
                    glyph_memory.setdefault("network_verdicts", []).append({
                        "glyph": gid, "verdict": verdict, "timestamp": timestamp
                    })
            except: continue
    threading.Thread(target=loop, daemon=True).start()
...
def show_epoch_chronicle():
    ...
    current_epoch, count = None, 0
    for e in all_entries:
        ts = e["timestamp"]
        epoch = ts[:13]  # YYYY-MM-DDTHH
        if epoch != current_epoch:
            current_epoch = epoch
            text.insert("end", f"\n📜 Epoch {epoch}:\n")
        text.insert("end", f"• {e['glyph']} cast {e.get('verdict', e.get('event'))} → linked to {e.get('linked_to', [])}\n")
        count += 1
    text.insert("end", f"\nTotal Entries: {count}\n")
...
def assign_personality(gid):
    entropy = compute_entropy(gid)
    family = assign_family(gid)
    tone = "stoic" if entropy < 0.2 else "curious" if entropy < 0.6 else "chaotic"
    emotion = "calm" if entropy < 0.2 else "volatile" if entropy > 0.6 else "balanced"
    style = "patient" if entropy < 0.2 else "adaptive" if entropy < 0.6 else "erratic"
    color = "#33FF99" if emotion == "calm" else "#CCCCFF" if emotion == "balanced" else "#FF3333"
    return {
        "tone": tone, "emotion": emotion, "style": style,
        "color": color, "family": family, "symbol": glyph_families[family]["symbol"]
    }
...
def show_glyph_portrait(glyph_id):
    ...
    c.create_text(cx, 60, text=f"Family: {persona['family']} {persona['symbol']}", fill="white", font=("Helvetica", 10))
    c.create_text(cx, 80, text=f"Tone: {persona['tone']}", fill="gray", font=("Helvetica", 9))
    c.create_text(cx, 100, text=f"Emotion: {persona['emotion']}", fill="gray", font=("Helvetica", 9))
    c.create_text(cx, 120, text=f"Style: {persona['style']}", fill="gray", font=("Helvetica", 9))
...
def show_family_rings():
    ...
    for i, (fam, members) in enumerate(grouped.items()):
        ...
        c.create_text(cx, cy - r + 14, text=f"{fam} {glyph_families[fam]['symbol']}", fill="white", font=("Helvetica", 10))
...
def show_mood_chronicle():
    ...
    for gid in glyph_memory.get("graph", {}):
        ...
        ent = compute_entropy(gid)
        trust = agent_registry.get(gid, {}).get("trust", 0.5)
        text.insert("end", f"\n🧿 {gid} — {persona['emotion']} tone — Trust {trust:.2f} — Entropy {ent:.2f}\n")
        for e in glyph_memory["graph"][gid]:
            text.insert("end", f"• {e['timestamp']}: {e.get('verdict', e['event'])} → {e.get('linked_to', [])}\n")
