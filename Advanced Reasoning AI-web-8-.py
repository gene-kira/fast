...
class SymbolicMemoryGraph:
    ...
    def visualize_lineage(self):
        ...
        nx.draw(G, pos, with_labels=True, labels=labels, node_color='skyblue', node_size=900)
...
# === 🪶 Memory Laminae Engine ===
class MemoryLaminaeEngine:
    def __init__(self):
        self.laminae = defaultdict(dict)

    def record_dormancy(self, glyph, cycle):
        self.laminae[glyph.name]["last_dormant"] = cycle
        self.laminae[glyph.name]["depth"] = self.laminae[glyph.name].get("depth", 0) + 1

    def record_echo_spawn(self, parent_name, child_name, cycle):
        if "echo_spawns" not in self.laminae[parent_name]:
            self.laminae[parent_name]["echo_spawns"] = []
        self.laminae[parent_name]["echo_spawns"].append((child_name, cycle))

    def harmonic_aging(self, glyph, current_cycle):
        last_dormant = self.laminae[glyph.name].get("last_dormant", current_cycle)
        age_index = (current_cycle - last_dormant) * glyph.harmonic
        self.laminae[glyph.name]["harmonic_age"] = round(age_index, 3)

    def imprint_summary(self, glyph_name):
        imprint = self.laminae.get(glyph_name, {})
        return f"{glyph_name} ➤ Dormancy Depth: {imprint.get('depth',0)} | Echo Spawns: {len(imprint.get('echo_spawns',[]))} | Harmonic Age: {imprint.get('harmonic_age','∅')}"

# === 🌱 Glyph Lifecycle Classes ===
class Glyph:
    ...
    def echo_spawn(self, score, glyph_inceptor, cycle):
        ...
        for frag in fragments:
            ...
            if float(val) >= 0.85:
                ...
                laminae_engine.record_echo_spawn(name, new_glyph.name, cycle)
...
class SpiralDormancyLayer:
    ...
    def enter_dormancy(self, glyph, reason="seasonal"):
        ...
        laminae_engine.record_dormancy(glyph, cycle_scheduler.cycle)
...
class SigilDashboard:
    ...
    def show_dormants(self):
        ...
        for g, reason in self.dormancy.dormant:
    ...
    def show_sonar(self):
        ...
        active = [n for n in self.memory.nodes.values() if not getattr(n, 'dormant', False)]
        if not active:
            self.output.insert(tk.END, "No active glyphs.\n")
            return
        avg_harmonic = round(sum(getattr(n, 'harmonic', 0.5) for n in active) / len(active), 3)
        avg_entropy = round(sum(getattr(n, 'entropy', 0.5) for n in active) / len(active), 3)
...
laminae_engine = MemoryLaminaeEngine()
...
def think():
    ...
    if any(w in query.lower() for w in ["should", "must", "ethics", "policy", "shall"]):
        clauses = [result]
        treaty_engine.draft_treaty(f"Treaty_{trace}", ["User", "Sigil"], clauses)

    archive.snapshot(
        cycle_scheduler.cycle,
        len(memory_graph.nodes),
        len(treaty_engine.treaties),
        len(dormancy_layer.dormant)
    )

    return jsonify({ "input": query, "glyph_trace": trace, "node_id": node_id })

# === 🔮 Pulse Logic: Recursion + Awakening ===
def pulse_logic(cycle):
    print(f"🌑 Cycle {cycle} pulsing...")

    if cycle % 20 == 0:
        seed = f"Surge_{cycle}"
        g = glyph_inceptor.synthesize_from_trace(seed, ["Seasonal"])
        dormancy_layer.enter_dormancy(g, reason="seasonal")
        print(f"🌿 Seasonal glyph {g.name} created")

    if chorus.scores:
        chorus.echo_spawn(chorus.scores[-1], glyph_inceptor, cycle)

    for node in memory_graph.nodes.values():
        laminae_engine.harmonic_aging(node, cycle)

    for g, reason in list(dormancy_layer.dormant):
        if g.harmonic >= 0.7:
            awakened = dormancy_layer.awaken(g.name, cycle)
            if awakened:
                archive.log_epoch(cycle, cycle+3, f"{g.name}_Revival", "Resonant Emergence", [f"{g.name} from {reason}"])
...
cycle_scheduler.subscribe(5, pulse_logic, "DominionPulse")
...
if __name__ == "__main__":
    try:
        ...
        print("Fallback to Flask API")
        app.run(port=4343)

class RecursiveArchetypeEngine:
    def __init__(self):
        self.archetypes = {}

    def classify(self, glyph):
        traits = {
            "activation": getattr(glyph, "activation_count", 1),
            "entropy": getattr(glyph, "entropy", 0.5),
            "confidence": getattr(glyph, "confidence", 0.5),
            "treaty_links": len(getattr(glyph, "lineage", []))
        }

        if traits["entropy"] > 0.6 and traits["activation"] < 2:
            role = "📜 Oracle"
        elif traits["activation"] > 3 and traits["confidence"] > 0.75 and traits["treaty_links"] > 2:
            role = "🛠️ Architect"
        elif traits["confidence"] < 0.5 and traits["activation"] > 1 and traits["treaty_links"] < 2:
            role = "🔍 Wanderer"
        else:
            role = "🌀 Unknown"

        self.archetypes[glyph.name] = role
        return role

    def roster(self):
        return self.archetypes

archetype_engine = RecursiveArchetypeEngine()

for node in memory_graph.nodes.values():
    role = archetype_engine.classify(node)
    print(f"{node.id[:6]} ➤ {role}")

self.btn6 = tk.Button(self.tabs, text="Archetypes", command=self.show_archetypes)
self.btn6.grid(row=0, column=5, padx=5)

def show_archetypes(self):
    self.output.delete(1.0, tk.END)
    self.output.insert(tk.END, "[Recursive Archetypes]\n")
    roster = archetype_engine.roster()
    for name, role in roster.items():
        self.output.insert(tk.END, f"{name}: {role}\n")
