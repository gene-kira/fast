...
autoload(["flask", "flask_socketio", "pyttsx3", "cryptography", "datetime", "threading"])
...
class Vault:
    def __init__(self):
        self.trust_data = {
            ...
            "peer_map": {},
            "mutation_rules": [],
            "memory_trails": {},           # ← Mnemonic Fractals
            "echo_log": [],                # ← Echo Sigils
            "persona_shards": {}           # ← Mythopoeic Shards
        }
    ...
    def save_codex(self, glyph, origin="local"):
        ...
        # Adaptive Decay
        count = sum(1 for e in self.trust_data["codex_log"] if e["glyph"] == glyph)
        decay_duration = max(120, 600 - count * 10)
        self.trust_data["glyph_decay"][glyph] = time.time() + decay_duration

        # Echo Sigil
        if origin != "echo":
            echo_glyph = f"~{glyph}"
            self.trust_data["echo_log"].append({"glyph": echo_glyph, "ts": ts})
            self.codex_log.append({"glyph": echo_glyph, "ts": ts, "origin": "echo"})
...
class SigilOrchestrator:
    def __init__(self, kernel):
        self.routes = {
            ...
            "ζ": lambda: print(json.dumps(kernel.vault.trust_data["memory_trails"], indent=2)),      # Fractal trails
            "ε": lambda: print(kernel.vault.trust_data["echo_log"]),                                 # Echoes
            "θ": lambda: print(json.dumps(kernel.vault.trust_data["persona_shards"], indent=2))      # Shards
        }
...
def ritual_ui():
    ...
    <script> var socket = io(); function cast(g){ fetch("/sigil/" + g,{method:"POST"}); } 
    socket.on("glyph_casted", d => console.log("Cast:", d)); </script></body></html>
    """
    ...
    try:
        ...
        while True:
            ...
            if data:
                ...
                kernel.vault.trust_data["persona_shards"].setdefault(addr[0], {"shard": kernel.persona, "echoes": []})
                kernel.vault.trust_data["persona_shards"][addr[0]]["echoes"].append(data)
