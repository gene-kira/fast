...
class VerdictProfile:
    def __init__(self, url, trust_score, emotion, glyph, voice_tone, decay_rate):
        ...
        self.label = None
        self.aura = None
        self.last_status = True  # Assume website is up initially
    ...
    def decay_trust(self):
        while True:
            ...
            if not self.last_status:
                self.trust_score = max(self.trust_score - 0.05, 0)  # Faster decay if website is down
            else:
    ...
    def check_website_real_time(self):
        while True:
            response = check_website(self.url)
            if response != self.last_status:
                self.last_status = response
                if response:
                    messagebox.showinfo("Website Up", f"{self.url} is now up and running.")
                    ...
                    narrate(f"Alert: {self.url} is offline.", "whisper")
...
def check_website(url):
    try:
        ...
        if response.status_code == 200 and is_valid_ssl(url):
...
def is_valid_ssl(url):
    try:
        ...
        log_entry(f"SSL validation failed for {url}: {e}")
        return False
