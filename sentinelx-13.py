...
REQUIRED_LIBS = [
    "psutil", "pyttsx3", "tkinter", "sqlite3", "websockets",
    "asyncio", "torch", "matplotlib"
]

def autoload():
    for lib in REQUIRED_LIBS:
        try:
            if lib in ["sqlite3", "tkinter"]:
                continue  # Built-in
            __import__(lib)
        except ImportError:
...
def speak(text):
    ...
    try:
        with voice_lock:
            engine = pyttsx3.init('sapi5')  # Use Windows stable backend
            engine.setProperty('volume', max(voice_state["volume"], 0.1))  # Clamp volume
            engine.say(text)
            engine.runAndWait()  # Avoid using stop()
