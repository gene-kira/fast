...
required_libs = [
    "tkinter", "psutil", "gc", "random", "time", "os",
    "matplotlib", "speechrecognition"
]
...
for lib in required_libs:
    try:
        __import__(lib if lib != "tkinter" else "tkinter")
    except ImportError:
        print(f"Installing missing library: {lib}")
        auto_install(lib if lib != "speechrecognition" else "SpeechRecognition")
...
class MemoryOptimizer:
    ...
    def annotate_chart(self, event_text):
        timestamp = time.strftime("%H:%M:%S")
        self.event_markers.append((timestamp, event_text))
    ...
    def update_all(self):
        ...
        for ts, label in self.event_markers[-10:]:
            if ts in self.x_data:
                idx = self.x_data.index(ts)
                self.ax.annotate(label, (self.x_data[idx], self.y_data[idx]), textcoords="offset points",
                                 xytext=(0,10), ha='center', fontsize=8, color='red', arrowprops=dict(arrowstyle="->"))
    ...
    def optimize_memory(self):
        gc.collect()
        self.annotate_chart("Memory Optimized")
