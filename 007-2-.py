...
class MagicBoxSuperAgent:
    def __init__(self, root):
        ...
        self.whitelist = set()  # Initialize an empty set for whitelisted IPs
    ...
    def create_widgets(self):
        ...
        ttk.Button(self.root, text="📁 View Logs", command=self.view_logs_in_new_window).pack(pady=5)
        ...
        # Volume Control
        ttk.Label(self.root, text="🔊 Volume Control").pack()
        self.volume_scale = ttk.Scale(self.root, from_=0, to=100, orient='horizontal', length=200, command=self.set_volume)
        self.volume_scale.pack(pady=5)

        # Whitelist Input
        ttk.Label(self.root, text="Whitelist IP").pack()
        self.whitelist_entry = tk.Entry(self.root, width=30)
        self.whitelist_entry.pack(pady=5)
        ttk.Button(self.root, text="Add to Whitelist", command=self.add_to_whitelist).pack(pady=5)
    ...
    def view_logs_in_new_window(self):
        log_window = Toplevel(self.root)
        log_window.title("MagicBox ASI Logs")
        log_window.geometry("600x400")

        try:
            with open("magicbox.log", "r", encoding="utf-8") as f:
                data = f.read()
            text_widget = Text(log_window, height=25, width=70)
    ...
    def add_to_whitelist(self):
        ...
        if ip:
            ...
            self.output.insert(tk.END, f"Added {ip} to whitelist.\n")
            self.logger.log(f"Added {ip} to whitelist.")
            self.say(f"{ip} added to whitelist.")
    ...
    def update_threats(self):
        ...
        if not ips:
            ...
            for ip in ips:
                if ip in self.whitelist:  # Skip whitelisted IPs
                    safe = f"🔎 Whitelisted IP: {ip}"
    ...
    def set_volume(self, value):
        ...
        self.engine.setProperty('volume', volume_level)
        self.say(f"Volume set to {value}%")
