# === OracleCore_v2_FINAL.py ===
# === Part 1: Imports, ECC Scan, Symbolic Engine, Forecast ===
...
# === Part 2: Voice Engine, Real-Time Scan Loop, Dispatcher ===
...
voice_enabled = True
volume_level = 1.0
voice_style = "default"  # "default", "serene", "anxious"
...
# 🔁 Auto Scan Flags
scan_active = True
scan_interval_seconds = 30  # Default scan interval
scan_thread = None

def toggle_scan():
    global scan_active
    scan_active = not scan_active
    status = "ON" if scan_active else "OFF"
    speak(f"Auto scan toggled {status}.", mood="default")
...
# 🌀 Real-Time Scan Dispatcher
def scan_loop(callback):
    while True:
        if scan_active:
            glyph_type = np.random.choice(["single_bit", "multi_bit", "entropy_cluster"])
            callback("auto", glyph_type, f"Scheduled scan ({scan_interval_seconds}s)")
...
def handle_ecc_event(source, glyph_type, message):
    print(f"[{source.upper()}] ECC Event — {glyph_type}: {message}")
...
# === Part 3: GUI Interface with Control Panel ===
...
def start_gui():
    ...
    def toggle_auto_scan(btn):
        toggle_scan()
        text = "Auto Scan: ON 🔄" if scan_active else "Auto Scan: OFF 🛑"
        btn.config(text=text)

    # === Scan Scheduler Dropdown ===
