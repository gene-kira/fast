...
def apply_magicbox_theme(root):
    ...
    style.configure("Allow.TButton", background="blue", foreground="white")
    style.map("Allow.TButton", background=[("active", "#0044aa")])
    style.configure("Block.TButton", background="red", foreground="white")
    style.map("Block.TButton", background=[("active", "#aa0000")])
...
def play_voice_snip(tag):
    print(f"🔈 Persona voice trigger: {tag}")
    # You can hook into audio playback libraries here (e.g., playsound, pyttsx3)
...
class MagicBoxApp:
    ...
    def perform_scan(self, is_auto=False):
        ...
        for ip in new_ips:
            ...
            # 👇 Add buttons under each scan result
            button_frame = ttk.Frame(self.output_text)
            self.output_text.window_create(tk.END, window=button_frame)

            allow_btn = ttk.Button(button_frame, text="Allow", style="Allow.TButton", command=lambda ip=ip: self.allow_ip(ip))
            allow_btn.pack(side=tk.LEFT, padx=(0,5))

            block_btn = ttk.Button(button_frame, text="Block", style="Block.TButton", command=lambda ip=ip, mood=mood: self.block_ip(ip, mood))
    ...
    def allow_ip(self, ip):
        print(f"✅ IP allowed: {ip}")
        self.status_label.config(text=f"✅ Allowed: {ip}")
        play_voice_snip("allow")

    def block_ip(self, ip, mood):
        print(f"⛔ IP blocked: {ip}")
        self.status_label.config(text=f"⛔ Blocked: {ip} — {mood}")
        play_voice_snip("block")
